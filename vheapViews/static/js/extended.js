...
function getChunkBaseAddress(chunk) {
	if (chunk.bin.includes("tcache")) {
		return hInt(chunk.address) - (chunk.headerSize || 0x10);
	}
...
function getChunkBaseFD(chunk) {
	if (chunk.bin.includes("tcache")) {
		return hInt(chunk.fd) - (chunk.headerSize || 0x10);
	}
...
function hInt(hexstr) {
	        if(hexstr === undefined || hexstr === null || hexstr === "None") { return NaN; }
	        return parseInt(String(hexstr).replace("0x", ''), 16);
}
